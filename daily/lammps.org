...
** TODO 项目进展情况, 是否有望取得价值研究成果
   ...
   我这阶段所做的工作大部分也主要在lammps的主核调试和从核优化部分 -mieee
   - 首先主核经过调通能够正常运行, 经测试性能较太湖之光上有 10%左右的提升
   - 对于从核部分, 由于对于我优化的这个eff核函数模块 <基本是从 0 开始做的> , 所有主要计划还是三个方面的工作, 分别是计算在多核上的分布, rma 和向量化的使用
     其中计算分布, rma 基本已经完成
   ...
   当前已经将这个项目的一两个算例跑了起来, 并打算在性能优化和正确性调校之后, 分别将剩下的更大规模的算例跑通, 大规模10sec/step
